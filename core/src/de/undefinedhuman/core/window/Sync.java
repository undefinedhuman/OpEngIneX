package de.undefinedhuman.core.window;

import static org.lwjgl.glfw.GLFW.glfwGetTime;

/**
 * Found this here: http://forum.lwjgl.org/index.php?topic=6582.0
 * Have done slight variations.
 * <p>
...
class Sync {
    ...
    private static class RunningAvg {
        ...
        public void init(long value) {
            while (this.offset < this.slots.length) this.slots[this.offset++] = value;
        }
        ...
        public void dampenForLowResTicker() {
            if (this.avg() > DAMPEN_THRESHOLD)
                for (int i = 0; i < this.slots.length; i++) this.slots[i] *= DAMPEN_FACTOR;
        }

        public long avg() {
            long sum = 0;
            for (long slot : this.slots) sum += slot;
